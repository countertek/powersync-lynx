...
@property(nonatomic, strong) NativeSyncHttp* syncHttp;
@end

@implementation NativePowerSyncModule

+ (void)setSharedStreamEventSender:(id)sender {
  [NativeSyncHttp setSharedStreamEventSender:sender];
}
...
- (instancetype)initWithParam:(id)param {
  if (self = [super init]) {
    _syncHttp = [[NativeSyncHttp alloc] initWithEventSender:param];
  }
  return self;
}
...
- (void)httpFetch:(NSDictionary*)request callback:(void (^)(id))callback {
  [self.syncHttp fetch:request callback:callback];
}

- (void)httpFetchAbort:(NSString*)streamId callback:(void (^)(id))callback {
  [self.syncHttp abort:streamId callback:callback];
}
